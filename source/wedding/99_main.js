 	...
 	haddle_envent_fn : function(){
		...
		car2._on('success',function(){
			...
			setTimeout(function(){
				...
				// 切换停止视频的播放
				$('video').each(function(){
					$(".u-maskLayer-close").removeClass("show");
 	...
 	video_init : function(){
        ...
        $('.j-video').each(function(){
            ...
            $(video).on('pause',function(){
				...
				$(".u-maskLayer-close").removeClass("show");
//				if(!car2._audio_contorl){
//					car2.audio_contorl();
//				}
            });
        })

        $('.j-video .m-btnPlay').on('click',function(){
        	var video = $(this).next()[0];
			$(".u-maskLayer-close").addClass("show");
        	if(video.paused){
        		$(video).removeClass('f-hide');
        		//video.play();
        		$(this).hide();
        ...
        $('.u-maskLayer-close').on('click',function(){
			$('video').each(function(){
				$(".u-maskLayer-close").removeClass("show");
				$("video").addClass('f-hide');
				if(!this.paused) this.pause();
 ...
 */
	...
	lazy_img : function(){
		...
		lazyNode.each(function(){
			var self = $(this);
			if(self.is('img')){
				self.attr('src','http://arayzou.qiniudn.com/@/zhangwen/loading_large.gif');
			}else{
				// 把原来的图片预先保存下来
				...
				self.css({
					'background-image'	: 'url(http://arayzou.qiniudn.com/@/zhangwen/loading_large.gif)',
 ...
 */
	//插件启动函数
 	plugin : function(){
		...
		// 音符飘逸
		$('#coffee_flow').coffee({
			steams				: ["<img src='http://arayzou.qiniudn.com/@/zhangwen/audio_widget_01@2x.png' />","<img src='http://arayzou.qiniudn.com/@/zhangwen/audio_widget_01@2x.png' />"],
			steamHeight			: 100,
			steamWidth			: 44
		});
